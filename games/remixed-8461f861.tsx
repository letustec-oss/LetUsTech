  ...
  useEffect(() => {
    ...
    const startWave = (waveNum) => {
      ...
      // Waves 6-10: Gradual ramp up
        ...
        baseZombieCount = 10.5 + ((waveNum - 5) * 2.5); // 13, 15.5, 18, 20.5, 23
      }
      // Waves 11-20: Steady increase
      else if (waveNum <= 20) {
        baseZombieCount = 23 + ((waveNum - 10) * 4); // 27, 31, 35... 63
      }
      // Waves 21-30: Significant challenge
      else if (waveNum <= 30) {
        baseZombieCount = 63 + ((waveNum - 20) * 6); // 69, 75, 81... 123
      }
      ...
      const spawnInterval = setInterval(() => {
        if (game.zombiesLeftToSpawn > 0 && gameState === 'playing') {
          ...
          // Waves 6-15: Gradual increase
          else if (waveNum <= 15) {
            difficultyMultiplier = 1.40 + ((waveNum - 5) * 0.15); // 1.55, 1.70... 2.90
          }
          // Waves 16-25: Significant increase
          else if (waveNum <= 25) {
            difficultyMultiplier = 2.90 + ((waveNum - 15) * 0.25); // 3.15, 3.40... 5.40
          }
          // Waves 26+: Extreme scaling
          else {
          ...
          let speedMultiplier;
          if (waveNum <= 5) {
            speedMultiplier = 1; // No speed increase for first 5 waves
          } else if (waveNum <= 15) {
            speedMultiplier = 1 + ((waveNum - 5) * 0.04); // 1.04... 1.40
          } else {
          ...
          if (waveNum <= 10) {
            damageMultiplier = 1 + (waveNum * 0.03); // 1.03... 1.30
          } else if (waveNum <= 20) {
            damageMultiplier = 1.30 + ((waveNum - 10) * 0.06); // 1.36... 1.90
          } else {
          ...
          const zombie = {
            ...
            damage: zombieTypes[type].damage * damageMultiplier,
            wallDamage: zombieTypes[type].wallDamage * damageMultiplier,
            attackCooldown: 0,
            targetLock: null,
    ...
    const gameLoop = () => {
      ...
      if (game.zombies.length === 0 && game.zombiesLeftToSpawn === 0 && game.waveActive) {
        ...
        let waveBonus;
        if (wave <= 5) {
          waveBonus = 15 + (wave * 3); // 18, 21, 24, 27, 30
        } else if (wave <= 15) {
          waveBonus = 30 + ((wave - 5) * 4); // 34, 38, 42... 70
        } else {
      ...
      // Walls with CARTOON 3D rendering
      game.walls.forEach(wall => {
        ...
        let wallTexture;
        if (wall.tier === 1) {
          // Wood texture - CARTOON style
          wallTexture = ctx.createLinearGradient(screenX, screenY, screenX + wall.width, screenY + wall.height);
          wallTexture.addColorStop(0, '#D2691E');
          wallTexture.addColorStop(0.5, '#8B4513');
          ...
          ctx.lineWidth = 4;
          for (let i = 0; i < (wall.height > wall.width ? wall.height : wall.width); i += 25) {
          ...
          // Stone texture - CARTOON style
          wallTexture = ctx.createLinearGradient(screenX, screenY, screenX + wall.width, screenY + wall.height);
          wallTexture.addColorStop(0, '#9BA2A8');
          wallTexture.addColorStop(0.5, '#708090');
          ...
          // Metal texture - CARTOON style
          wallTexture = ctx.createLinearGradient(screenX, screenY, screenX + wall.width, screenY + wall.height);
          wallTexture.addColorStop(0, '#C0D6DF');
          wallTexture.addColorStop(0.5, '#8B9DC3');
          ...
          for (let py = 0; py < wall.height; py += panelSize) {
            for (let px = 0; px < wall.width; px += panelSize) {
              ...
              ctx.lineWidth = 2;
              const rivetPositions = [[3, 3], [panelSize-6, 3], [3, panelSize-6], [panelSize-6, panelSize-6]];
              rivetPositions.forEach(([rx, ry]) => {
                if (px + rx < wall.width && py + ry < wall.height) {
                  ctx.beginPath();
                  ctx.arc(screenX + px + rx, screenY + py + ry, 3, 0, Math.PI * 2);
                  ctx.fill();
                  ctx.stroke();
                }
              });
        ...
        if (wall.upgradeLevel > 0) {
          ...
          ctx.strokeText(starText, screenX + wall.width - 8, screenY + 18);
      ...
      // Turrets with enhanced CARTOON 3D rendering
      game.turrets.forEach(turret => {
        const turretSize = turret.size || 50;
        const centerX = turret.x + turretSize / 2;
        const centerY = turret.y + turretSize / 2;
        ...
        if (showRanges) {
          ...
          rangeGradient.addColorStop(0, turretColor + '50');
          rangeGradient.addColorStop(0.7, turretColor + '25');
        ...
        baseGradient.addColorStop(0.6, '#3a3a3a');
        ...
        platformGrad.addColorStop(1, darkenColor(turretColor, 30));
        ...
        if (turret.lastShotTime && Date.now() - turret.lastShotTime < 100) {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            const outerX = 30 + Math.cos(angle) * 12;
        ...
        if (turret.upgradeLevel > 0) {
          ...
          ctx.strokeText(starText, screenX + turretSize - 6, screenY + 20);
      ...
      game.traps.forEach(trap => {
        ...
        ctx.strokeStyle = '#000000';
        ctx.lineWidth = 4;
        ctx.strokeText('⚠️', screenX, screenY + 10);
        ctx.fillStyle = '#ffff00';
        ctx.fillText('⚠️', screenX, screenY + 10);
      });

      // Bases with CARTOON rendering
      game.bases.forEach(base => {
        ...
        baseGradient.addColorStop(0, '#ffa726');
        baseGradient.addColorStop(0.7, '#f39c12');
        ...
        if (base.armor > 0) {
          for (let i = 0; i < base.armor; i++) {
            ctx.strokeStyle = `rgba(255, 215, 0, ${0.4 + i * 0.15})`;
            ctx.lineWidth = 4;
        ...
        castleGrad.addColorStop(0.5, '#b8956a');
        ...
        for (let i = 0; i < 5; i++) {
          if (i % 2 === 0) {
            ...
            ctx.fillRect(castleX + i * battlmentWidth, castleY - 10, battlmentWidth, 10);
            ctx.strokeStyle = '#000000';
            ctx.lineWidth = 3;
        ...
        ctx.lineWidth = 3;
        for (let i = 0; i < 2; i++) {
          const winX = castleX + castleWidth * (0.25 + i * 0.5);
          const winY = castleY + castleHeight * 0.3;
          ctx.beginPath();
          ctx.arc(winX, winY, 8, 0, Math.PI * 2);
          ctx.fill();
          ctx.stroke();
        }
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(castleX + castleWidth * 0.75, castleY - 30);
        ctx.lineTo(castleX + castleWidth * 0.95 + flagWave * 3, castleY - 25);
        ctx.lineTo(castleX + castleWidth * 0.95 + flagWave * 3, castleY - 15);
        ctx.lineTo(castleX + castleWidth * 0.75, castleY - 10);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        ...
        if (healthPercent < 0.3) {
          ...
          ctx.strokeStyle = '#000';
          ctx.lineWidth = 4;
          ctx.strokeText('⚠️ CRITICAL!', screenX + base.size/2, screenY - 40);
          ctx.fillText('⚠️ CRITICAL!', screenX + base.size/2, screenY - 40);
          ctx.globalAlpha = 1;
        }
      });
      ...
      // Zombies with enhanced CARTOON visuals
      game.zombies.forEach(zombie => {
        ...
        if (screenX > -50 && screenX < canvas.width + 50 && screenY > -50 && screenY < canvas.height + 50) {
          ...
          zombieGradient.addColorStop(0.6, zombie.color);
          zombieGradient.addColorStop(1, darkenColor(zombie.color, 20));
          ...
          if (zombie.type === 'boss' || zombie.type === 'megaboss') {
            ...
            ctx.lineWidth = zombie.type === 'megaboss' ? 6 : 4;
            ctx.shadowBlur = (zombie.type === 'megaboss' ? 15 : 12) * glowIntensity;
          ...
          ctx.fillStyle = '#ffffff';
          for (let i = 0; i < 4; i++) {
            const toothX = screenX - zombie.size/4 + (i * zombie.size/6);
          ...
          if (zombie.targetLock && typeof zombie.targetLock === 'string') {
            ...
            if (iconText) {
              ...
              ctx.strokeStyle = '#000';
              ctx.lineWidth = 3;
              ctx.strokeText(iconText, screenX, screenY + zombie.size + 28);
          ...
          if (zombie.type === 'healer') {
            ...
            ctx.strokeStyle = `rgba(46, 204, 113, ${0.5 * healPulse})`;
            ctx.lineWidth = 4;
            ctx.beginPath();
            ...
            if (zombie.healCooldown > 0) {
              ...
              ctx.shadowBlur = 25 * activeGlow;
              ctx.shadowColor = '#2ecc71';
              ctx.strokeStyle = `rgba(46, 204, 113, ${0.9 * activeGlow})`;
              ctx.lineWidth = 5;
              ctx.beginPath();
            ...
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 4;
            ctx.strokeText('✚', screenX, screenY - zombie.size - 8);
            ctx.fillText('✚', screenX, screenY - zombie.size - 8);
            ctx.shadowBlur = 0;
          } else if (zombie.type === 'armored') {
            // Armor plating - CARTOON style
            ctx.strokeStyle = '#95a5a6';
            ctx.lineWidth = 5;
            ctx.setLineDash([8, 4]);
            ctx.beginPath();
            ctx.arc(screenX, screenY, zombie.size + 3, 0, Math.PI * 2);
            ctx.stroke();
            ctx.setLineDash([]);
            ...
            if (healthPercent < 0.5) {
              ...
              ctx.strokeStyle = `rgba(231, 76, 60, ${0.6 * rageIntensity})`;
              ctx.lineWidth = 4;
              ctx.beginPath();
            ...
            ctx.shadowBlur = 25 * necroGlow;
            ctx.shadowColor = '#9b59b6';
            ctx.strokeStyle = `rgba(155, 89, 182, ${0.5 * necroGlow})`;
            ctx.lineWidth = 4;
            ctx.beginPath();
            ctx.arc(screenX, screenY, zombie.size + 10, 0, Math.PI * 2);
            ctx.stroke();
            ...
            // Shield indicator - CARTOON
            ctx.fillStyle = 'rgba(22, 160, 133, 0.4)';
      ...
      // Bullets with enhanced CARTOON visuals and trails
      game.bullets.forEach(bullet => {
        ...
        if (bullet.homing) {
          ...
          ctx.lineWidth = 3;
          ctx.beginPath();
          ctx.arc(screenX, screenY, bulletSize + 5, 0, Math.PI * 2);
          ctx.stroke();
          ...
          for (let i = 0; i < 4; i++) {
            ...
            ctx.moveTo(screenX, screenY);
            ctx.lineTo(
              screenX + Math.cos(angle) * (bulletSize + 4),
              screenY + Math.sin(angle) * (bulletSize + 4)
            );
            ctx.stroke();
          }
        }
        ...
        if (bullet.fromTurret) {
          ctx.globalAlpha = 0.4;
          ctx.beginPath();
          ctx.arc(screenX, screenY, bulletSize + 8, 0, Math.PI * 2);
          ctx.fill();
          ctx.globalAlpha = 1;
        }
        ...
        if (bullet.isCrit) {
          ...
          for (let i = 0; i < 8; i++) {
            ...
            const sparkleX = screenX + Math.cos(angle) * 10;
            const sparkleY = screenY + Math.sin(angle) * 10;
          ...
          for (let i = 0; i < 4; i++) {
            ctx.beginPath();
            ctx.moveTo(0, 0);
            ctx.lineTo(Math.cos(i * Math.PI / 2) * 12, Math.sin(i * Math.PI / 2) * 12);
            ctx.lineTo(Math.cos((i + 1) * Math.PI / 2) * 12, Math.sin((i + 1) * Math.PI / 2) * 12);
            ctx.fill();
            ctx.stroke();
          }
          ctx.restore();
      ...
      // Player with enhanced CARTOON visuals
      ...
      const bodyGradient = ctx.createRadialGradient(
        ...
        0, 0, game.player.size * 1.3
      );
      bodyGradient.addColorStop(0, '#ff9999');
      ...
      if (Date.now() - game.lastShot < 100) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const innerRadius = 7;
          const outerX = game.player.size * 1.5 + Math.cos(angle) * outerRadius;
      ...
      const effectIconWidth = 30;
      const maxEffectsPerRow = 6;
      ...
      const drawEffect = (icon, color, width = effectIconWidth) => {
        ...
        ctx.fillStyle = color;
        ctx.fillText(icon, effectsX, effectsY);
        effectsX += width;
        effectCount++;
      };
      ...
      if (game.player.fortify > 0) drawEffect(`🛡️x${Math.floor(game.player.fortify / 0.2)}`, '#4169e1', 40);
      if (game.player.lucky) drawEffect('🍀', '#00ff00');
      if (game.player.secondChance && !game.player.secondChanceUsed) drawEffect('💫', '#ffff00');
      ...
      {gameState === 'menu' && (
        <div className="bg-black/80 rounded-xl p-8 border-4 border-cyan-500 text-center max-w-2xl">
          ...
          <div className="text-left text-white mb-6 space-y-2 bg-black/50 p-4 rounded">
            ...
            <p className="ml-4">• TAB - Works without scrolling page!</p>
          ...
          <div className="text-left text-white mb-6 space-y-2 bg-black/50 p-4 rounded">
            ...
            <p className="ml-4 text-pink-500">• 🎨 <span className="font-bold">CARTOON GRAPHICS!</span> - Vibrant colors, thick outlines, expressive characters!</p>
            <p className="ml-4 text-green-400">• ⚖️ <span className="font-bold">REBALANCED!</span> - Easier early waves (1-5), progressively harder late game!</p>
            <p className="ml-4 text-cyan-400">• 🎮 <span className="font-bold">Better Controls!</span> - TAB no longer scrolls, works with WASD!</p>
            <p className="ml-4 text-purple-400">• 🎭 <span className="font-bold">Expressive Zombies!</span> - Big eyes, cartoon mouths, unique designs for all 17 types!</p>
            ...
            <p className="ml-4 text-cyan-400">• 🏗️ <span className="font-bold">Cartoon Style!</span> - Round friendly characters, vibrant colors, thick black outlines!</p>
